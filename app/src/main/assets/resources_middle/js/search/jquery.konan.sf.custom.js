/*
 * 2018-06-22 *
 * api 조회 결과를 컨트롤러에서 받아올 때 json이 아닌 string 데이터를 가져와서 json화 시키도록 변경
 *
 *
 * 2018-07-04
 * api를 통해 가져온 키워드들에 goSearch() 함수 적용
 *
 */
...
//		$("<span class=\"ui-icon ui-icon-triangle-1-s\"></span>").insertAfter($('#auto_span')).position({my:"right-10 center", at:"right center", of:$('#auto_span')});
...
//			$(element).prepend(this.menu.element);
		}
...
//						console.log(data);
	...
	_normalize: function( items ) {
		if ( items.seed && items.suggestions ) {
			...
			var merge = $.merge(items.suggestions[0], items.suggestions[1]);
...
//		$(ul).appendTo('div.inner');
	...
	_renderItem: function( ul, item ) {
		if(item[0].indexOf("##@@##") != -1){
			return $( "<li>" ).html(item[0].split("##@@##")[1]).appendTo(ul);
		}

		return $( "<li>" )
			.append( (item.length > 1)
				? $( "<a>" ).attr( "href", item[1] ).html( this.highlight( item[0], this.seed ) )
				: $( "<a>" ).attr("href", 'javascript:goSearch(null, "'+ item[0] +'")').html( item[0]/*this.highlight( item[0], this.seed )*/ ) )
			.click(function(e){
				e.stopPropagation();
			})
			.attr('q-data', item[0])
