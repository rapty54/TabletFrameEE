...
var drawVideoPanel = function(VIDEO_PNODE, idx, zoom) {
    ...
    videoCoverBtn.style.background = "url(\"../../resources/img/video/ico_play.png\") no-repeat center center";
	...
	videoCoverBtn.addEventListener("mouseover", function() {
		if($("body").hasClass("fullscreen")){
			...
			if (video.paused == true) {
				...
				videoCoverBtn.style.background = "url(\"../../resources/img/video/ico_pause.png\") no-repeat center center";
			}
		}
	});
    ...
    */
	...
	$('#videoSeek'+idx).slider({
        value: 0,
        orientation: "horizontal",
        range: "min",
        min:0,
        max:video.duration,				
				slide: function(event, seekto){
					
					video.currentTime = seekto.value;
				},
	...
	video.ontimeupdate = function() {
			...
			//$('#videoSeek'+idx).slider("value", video.currentTime);
	};

	
	video.addEventListener("timeupdate", function() {		
	  var value = (100 / video.duration) * video.currentTime;	  
		$('.ui-slider-range','#videoSeek'+idx).css({"width" : value + "%"});
		$('.ui-slider-handle','#videoSeek'+idx).css({"left" : value + "%"});
	...
	function funcFullScreen(){
		...
		if (isIOS) {
			$("body").addClass("is-video-controls");
	...
	function funcStopVideo(){
		...
		//seekBar.value = 0;
			...
			$('.ui-slider-range','#videoSeek'+idx).css({"width" : "0%"});
			$('.ui-slider-handle','#videoSeek'+idx).css({"left" : "0%"});
    ...
    function funcEndVideo(){
        ...
        playButton.classList.remove('pause');
        playButton.innerHTML = "Play";
        $('.ui-slider-range','#videoSeek'+idx).css({"width" : "100%"});
				$('.ui-slider-handle','#videoSeek'+idx).css({"left" : "100%"});
        videoCoverBtn.style.background = "url(\"../../resources/img/video/ico_play.png\") no-repeat center center";        
    }
