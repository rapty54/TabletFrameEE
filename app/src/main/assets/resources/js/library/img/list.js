$(function () {
	var reset = false;
	var photoIdx = $('input[name=photoIdx]').val();
	if (type1 != "") {
	...
	/* 2023 2학기 초등개편 포토존(라이브러리) 주석처리
	var getPhotoZone = function() {

		var $tabConts = null;
		$tabConts = $('#photozone');
		Ajax.execute({
			url: '/library/img/photo/zone/data',
			data: {
	...
	*/
	...
	$('.btnSrcDetail').on('click', function() {
		$(this).parent('.srcDetail').toggleClass('on');
	});
});
