$(function () {
		...
		$('#content').on('click','.detailPage',function(){
			var page  = $('#content').find('input[name=number]').val();
			if (SessionUtils.isLogin(location.href)) {
				location.href = "/class/management/huhsam/detail?boardId=" + $(this).data('seq') + "&page=" + page;
			}
		});
		...
		$('#btn-teacher-about-layer').bind('click', function() {
			Layer.openLayer({
				url: '/class/management/huhsam/teacher/about.popup',
	...
	function getList(category) {
		...
		$tabConts = $('#content');
		Ajax.execute({
			url: '/class/management/huhsam/content.html',
