$(function () {
	...
	$('#list').on('click','.detailPage',function(){
	   ...
	   if (SessionUtils.isLogin(location.href)) {
	      location.href = "/class/creative/artKnowledge/detail?boardId=" + $(this).data('seq')+"&page="+page;
	   }
	});
	...
	$('#btn-teacher-about-layer').bind('click', function() {
		Layer.openLayer({
			url: '/class/creative/artKnowledge/teacher/about.popup',
...
function getList() {
	...
	$tabConts = $('#list');
	Ajax.execute({
		url: '/class/creative/artKnowledge/artKnowledgeList',
		data: {
