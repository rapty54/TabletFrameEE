...
//			if ($('.nt_lineDragArea .dropItem.on').length < newType.lineDrawNumAnswer) {
	...
	// --add
	lineDrawQuiz3: function (_numAnswer) {
		newType.lineDrawNumAnswer = _numAnswer;
		newType.completeCnt = 0;
...
//			if ($('.nt_lineDragArea .dropItem.on').length < newType.lineDrawNumAnswer) {
		...
		$(".nt_lineDragArea .dropItem").droppable({
			drop: function (event, ui) {
				clearTimeout(setT3)
				ado_stop()
				var drag_id = $(this).attr('data-match-id')
				var drop_id = newType.lineDrawDragObj.attr('data-match-id')
				if(drag_id === drop_id){
					newType.lineDrawDropObj = $(this);
					COMMONLIBRARY.tools.correctAudio();
					$('.page svg line').eq(newType.lineDrawDragObj.index()).addClass('com')
				}
				else{
					COMMONLIBRARY.tools.wrongAudio();
					// return false;
				...
				if($(".nt_lineDragArea .dropItem.on").length  === $(".nt_lineDragArea .dropItem").length - 1){
					$('.nt_lineWrap').addClass('com')
