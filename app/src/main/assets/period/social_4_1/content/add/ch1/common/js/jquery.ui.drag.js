...
//    if (!isMobile) factor = parent.ZOOMVALUE;
	...
	return function( draggable, droppable, toleranceMode, event ) {
		...
		var x1 = (( draggable.positionAbs || draggable.position.absolute ).left / factor) + draggable.margins.left,
			...
			$('.jdrag').css({
				'top' : ( draggable.positionAbs || draggable.position.absolute ).top,
				'left' : ( draggable.positionAbs || draggable.position.absolute ).left,
				'width' : draggable.helperProportions.width,
				'height' : draggable.helperProportions.height
			});

			$('.jdrop').css({
				'position' : 'absolute',
				'top' : droppable.offset.top,
				'left' : droppable.offset.left,
				'width' : droppable.proportions().width,
				'height' : droppable.proportions().height
			})
