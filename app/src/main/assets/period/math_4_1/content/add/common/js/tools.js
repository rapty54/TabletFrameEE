...
COMMONLIBRARY.tools = (function () {
  ...
  return {
    ...
    effectBox: function () {
      ...
      var soundUrl = '../../../common/media/click.mp3';
      ...
      soundUrl = '../../../common/media/feed_ok.mp3';
      ...
      soundUrl = '../../../common/media/feed_no.mp3';
      ...
      soundUrl = '../../../common/media/paging.mp3';
    ...
    slider: function () {
      ...
      //네비바 넣기 2021-12-06-KSY (dot에 on부여 410번줄)
      var i;
      var j;
      var sl = $(slide).children();
      var navi = '<div class="navigation"></div>';
      var dot = '<div class="dot"></div>';

      sl.append(navi);
      
      for(i=0; i<cnt; i++){ // 각 slide에 dot 적용 
        for(j=0; j<cnt; j++){ // 각 slide안의 dot 갯수 적용
          sl.eq(i).find('.navigation').append(dot);
        }
      }

      $(slide).find("li.slide").first().find('.dot').first().addClass("on");

      sl.find('.navigation').css('width', (51 * cnt) + 'px');
      ...
      $(document).on('click','.btnNext',function(){
        ...
        /*네비바 on부여 2021-12-06-KSY*/
        $(sliderWrap).find(".slide").eq(now).find('.dot').eq(now).addClass("on");
    ...
    popLayer: function () {
      ...
      $(".pop_btnNext").click(function(){
        ...
        //next 버튼 dim 여부 처리
        now++;
        // var len = $(pop_slideList).find(".pop_slide").children().length; 2021-12-08-KSY 수정
        var len = $(pop_slideList).find(".pop_slide").length;
      ...
      /*팝업창(슬라이더버전) 네비바 2021-12-08-KSY*/
      var i;
      var j;
      var sl = $('.pop_slideList').children();
      var cnt = $('.pop_slideList').children().length;
      var navi = '<div class="navigation"></div>';
      var dot = '<div class="dot"></div>';
      var idx = $('.pop_sliderWrap').find(".pop_slide.on").index();
      ...
      sl.find('.navigation').css('width', (51 * cnt) + 'px');
      $('.pop_slideList').find("li.pop_slide").first().find('.dot').first().addClass("on");

      for(idx=0; idx<cnt; idx++){
        $('.pop_sliderWrap').find("li.pop_slide").eq(idx).find('.dot').eq(idx).addClass("on");
      } 
...
function dataOrderFix(){
  ...
  function playSoundOk(){
      //Sound        
    var clickAudio = document.getElementsByClassName('btnSoundOk')[0];
    if (!clickAudio.ended) {
       clickAudio.currentTime = 0;
    }
    clickAudio.play();
  ...
  function playSoundNo(){
      //Sound        
    var clickAudio = document.getElementsByClassName('btnSoundNo')[0];
    if (!clickAudio.ended) {
       clickAudio.currentTime = 0;
    }
    clickAudio.play();
  ...
  function playSoundPaging(){
      //Sound        
    var clickAudio = document.getElementsByClassName('btnSoundPaging')[0];
    if (!clickAudio.ended) {
       clickAudio.currentTime = 0;
    }
    clickAudio.play();
