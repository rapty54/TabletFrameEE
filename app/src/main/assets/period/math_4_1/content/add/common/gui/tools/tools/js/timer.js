...
$(function(){
	var max_m = 99;
	if(timer_pie) {max_m = 60;}
	var max_s = 59;
    var timer_id = 0;
    var target_time = new Date();
    var max_time = max_m*60 + 59;
    if(timer_pie) {max_time = max_m*60;}
    var play = false;
    var min_height = WIN_MIN_HEIGHT;
    if(timer_pie || timer_analog) {min_height = WIN_MIN_HEIGHT2;}
	...
	$("#btn_30s").on("click", function(event) { add_s(30); });
	$("#btn_1m").on("click", function(event) { add_m(1); });
    $("#btn_3m").on("click", function(event) { add_m(3); });
    $("#btn_5m").on("click", function(event) { add_m(5); });
	...
	function init_time(){
	    ...
	    if(timer_pie){
	        $(".half .fill").eq(0).css({transform:"rotate(0deg)"});
            $(".half .fill").eq(1).css({transform:"rotate(0deg)"});
	    } else if(timer_analog){
	        $("#sec_r").hide();
	        $("#min_r").hide();
	        $("#hour_r").hide();
    ...
    function started(){
        ...
        if(timer_analog){  display_analog_target(); }
        make_target_time();
