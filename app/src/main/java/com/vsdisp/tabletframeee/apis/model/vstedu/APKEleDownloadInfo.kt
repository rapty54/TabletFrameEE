 ...
 * Full URL : https://smart-dn.visang.com/appData/vstedu/info/apk/ele/apk_ele_info.json
 */
@Keep
data class APKEleDownloadInfo(
    val apk: Apk, // 다운 로드 받을 APK 상세 정보
    val contentRootPath: String, // host 뒤쪽 자원 위치 Root Path
    val contentsType: String, // 초등(ele) / 중고등(mid_hig) 구분
) {
    data class Apk(
        val dInfo: DInfo,
    ) {
