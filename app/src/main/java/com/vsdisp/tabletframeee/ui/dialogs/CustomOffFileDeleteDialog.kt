...
import com.vsdisp.tabletframeee.databinding.DialogContentsOfflineDeleteBinding
...
class CustomOffFileDeleteDialog(
    ctx: AppCompatActivity,
    itemOffInfo: OffItemDetail,
    deleteComplete: (ActDeleteResultData) -> Unit,
) {

    private var mItemInfo = itemOffInfo

    private var binding: DialogContentsOfflineDeleteBinding =
        DialogContentsOfflineDeleteBinding.inflate(ctx.layoutInflater)
    ...
    init {
        ...
        val txContentTitle = layer.deleteOfflineContentTitle
        val txDescription = layer.deleteOfflineTextDescription

        cInstance.launch(Dispatchers.Main) {
            txContentTitle.text = "삭제 항목 : " + mItemInfo.title
            txDescription.text = "잠시만 기다려 주세요\n해당 파일 삭제 중 입니다."
        }

        var deviceSavedPath = mItemInfo.downloadDevicePath
        var fileNameWithExt = mItemInfo.filenameWithExt
    ...
    private fun returnLayer(): DialogContentsOfflineDeleteBinding {
        return binding
    }
